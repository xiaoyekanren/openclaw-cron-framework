...
```bash
${CRON_DIR:-${HOME:-/home/zzm}/.openclaw/cron}/task-framework.sh list
```

### 2. 执行单个任务

```bash
${CRON_DIR:-${HOME:-/home/zzm}/.openclaw/cron}/task-framework.sh run <task_name>
```

### 3. 执行所有任务

```bash
${CRON_DIR:-${HOME:-/home/zzm}/.openclaw/cron}/task-framework.sh run-all
```

### 4. 查看任务状态

```bash
${CRON_DIR:-${HOME:-/home/zzm}/.openclaw/cron}/task-framework.sh status
```
...
在 `${CRON_DIR:-${HOME:-/home/zzm}/.openclaw/cron}/` 目录下创建你的脚本，例如：

```bash
#!/bin/bash
# ${CRON_DIR:-${HOME:-/home/zzm}/.openclaw/cron}/backup-database.sh
...
```bash
# ${CRON_DIR:-${HOME:-/home/zzm}/.openclaw/cron}/tasks/backup-db.task

TASK_DESC="数据库备份"
TASK_SCRIPT="${CRON_DIR:-${HOME:-/home/zzm}/.openclaw/cron}/backup-database.sh"
```

### 步骤 3: 测试任务

```bash
${CRON_DIR:-${HOME:-/home/zzm}/.openclaw/cron}/task-framework.sh run backup-db
```
...
# 添加任务（每天凌晨 2 点执行）
0 2 * * * ${CRON_DIR:-${HOME:-/home/zzm}/.openclaw/cron}/task-framework.sh run backup-db
```
...
```bash
# 查看示例
cat ${CRON_DIR:-${HOME:-/home/zzm}/.openclaw/cron}/crontab.example

# 应用到 crontab
cat ${CRON_DIR:-${HOME:-/home/zzm}/.openclaw/cron}/crontab.example | crontab -
```
...
export PATH="/usr/local/node-v24.14.0-linux-x64/bin:${HOME:-/home/zzm}/.npm-global/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin"
```
...
- **日志位置**: `${CRON_DIR:-${HOME:-/home/zzm}/.openclaw/cron}/logs/`
- **自动清理**: 保留 7 天，自动删除旧日志
- **查看详细日志**: `cat ${CRON_DIR:-${HOME:-/home/zzm}/.openclaw/cron}/logs/<task_name>_<timestamp>.log`
...
4. 手动测试：`${CRON_DIR:-${HOME:-/home/zzm}/.openclaw/cron}/task-framework.sh run <task_name>`
...
- `${HOME:-/home/zzm}/.openclaw/workspace/memory/2026-03-17-cron-best-practices.md` - 详细指南
