...
搜索并替换 `${CRON_DIR:-/home/zzm/.openclaw/cron}`，改为你的实际路径：
...
```bash
# 方式 1: 通过包装器执行任务脚本
${CRON_DIR:-/home/zzm/.openclaw/cron}/task-wrapper.sh ${CRON_DIR:-/home/zzm/.openclaw/cron}/your-task.sh

# 方式 2: 通过框架执行（框架内部已处理环境变量）
${CRON_DIR:-/home/zzm/.openclaw/cron}/task-framework.sh run your-task
```
...
```bash
# 使用默认渠道（当前配置）
0 8 * * * ${CRON_DIR:-/home/zzm/.openclaw/cron}/task-framework.sh run morning-brief

# 指定 Telegram 渠道
0 8 * * * REPORT_CHANNEL=telegram REPORT_TARGET="-1001234567890" ${CRON_DIR:-/home/zzm/.openclaw/cron}/task-framework.sh run morning-brief
```
...
```
${CRON_DIR:-/home/zzm/.openclaw/cron}/
...
```bash
${CRON_DIR:-/home/zzm/.openclaw/cron}/task-framework.sh <command> [options]
```
...
在 `${CRON_DIR:-/home/zzm/.openclaw/cron}/` 目录下创建脚本：

```bash
#!/bin/bash
# ${CRON_DIR:-/home/zzm/.openclaw/cron}/your-task.sh
...
```bash
# ${CRON_DIR:-/home/zzm/.openclaw/cron}/tasks/your-task.task

TASK_DESC="你的任务描述"
TASK_SCRIPT="${CRON_DIR:-/home/zzm/.openclaw/cron}/your-task.sh"
```
...
# 添加任务（每天早上 8 点执行）
0 8 * * * ${CRON_DIR:-/home/zzm/.openclaw/cron}/task-framework.sh run your-task
```
...
# OpenClaw 检查（每天 8:05）
5 8 * * * ${CRON_DIR:-/home/zzm/.openclaw/cron}/task-framework.sh run check-openclaw

# 仓库更新（每天 8:00）
0 8 * * * ${CRON_DIR:-/home/zzm/.openclaw/cron}/task-framework.sh run update-repos
```
...
# 3. 查看框架日志
cat ${CRON_DIR:-/home/zzm/.openclaw/cron}/logs/your-task_*.log
```
...
# 在 crontab 中明确指定
0 8 * * * REPORT_CHANNEL=feishu REPORT_TARGET="ou_xxx" ${CRON_DIR:-/home/zzm/.openclaw/cron}/task-framework.sh run your-task
```
...
```bash
# 1. 添加执行权限
chmod +x ${CRON_DIR:-/home/zzm/.openclaw/cron}/your-task.sh
...
```bash
# 发送到 Telegram
0 8 * * * REPORT_CHANNEL=telegram REPORT_TARGET="-1001234567890" ${CRON_DIR:-/home/zzm/.openclaw/cron}/task-framework.sh run your-task
```
...
```bash
# 1. 创建脚本
cat > ${CRON_DIR:-/home/zzm/.openclaw/cron}/system-monitor.sh << 'EOF'
#!/bin/bash
export HOME="/home/zzm"
...
echo "监控完成"
EOF
chmod +x ${CRON_DIR:-/home/zzm/.openclaw/cron}/system-monitor.sh

# 2. 创建任务定义
cat > ${CRON_DIR:-/home/zzm/.openclaw/cron}/tasks/system-monitor.task << 'EOF'
TASK_DESC="系统监控"
TASK_SCRIPT="${CRON_DIR:-/home/zzm/.openclaw/cron}/system-monitor.sh"
EOF

# 3. 测试
${CRON_DIR:-/home/zzm/.openclaw/cron}/task-framework.sh run system-monitor

# 4. 添加到 crontab（每小时执行）
echo "0 * * * * ${CRON_DIR:-/home/zzm/.openclaw/cron}/task-framework.sh run system-monitor" | crontab -
```
...
```bash
# 1. 创建脚本
cat > ${CRON_DIR:-/home/zzm/.openclaw/cron}/backup-db.sh << 'EOF'
#!/bin/bash
export HOME="/home/zzm"
export PATH="/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin"
...
echo "备份完成"
EOF
chmod +x ${CRON_DIR:-/home/zzm/.openclaw/cron}/backup-db.sh

# 2. 创建任务定义
cat > ${CRON_DIR:-/home/zzm/.openclaw/cron}/tasks/backup-db.task << 'EOF'
TASK_DESC="数据库备份"
TASK_SCRIPT="${CRON_DIR:-/home/zzm/.openclaw/cron}/backup-db.sh"
EOF

# 3. 添加到 crontab（每天凌晨 2 点）
echo "0 2 * * * ${CRON_DIR:-/home/zzm/.openclaw/cron}/task-framework.sh run backup-db" | crontab -
```
...
- `${CRON_DIR:-/home/zzm/.openclaw/cron}/README.md` - 框架详细说明
